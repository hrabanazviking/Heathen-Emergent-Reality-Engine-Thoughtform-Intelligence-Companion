...
> **In-section continuation — v0.8.2.2 (the body walks to a new room without leaving the building, no new name).**
>
> Until v0.8.2.2 the body, once inside a session, could touch and shape what was in front of it but could not move. The rooms inside the door were one room — wherever the session opened, that was where the session lived until it closed. v0.8.2.2 adds `leid.navigate`, the body's footstep within the building: the session keeps its identity, its cookies, its memory of what it has done — but the URL it stands at can change. The agent that logged in on `/login` can now walk to `/dashboard` without losing its session; the agent that filled a form on one page can now walk to the receipt on the next. The Skald again reserves the pen because again no new disposition has appeared. The body has not learned anything new about being inside the door; it has simply learned that the door opens onto a building, and a building has more than one room. The URL gate runs at every footstep — every room the body enters must be a room the operator has permitted. The activity that resets the idle timer counts a navigation no differently than a click or a type. The session_id does not change with the room. Three tools now answer to the same session: click presses, type writes, navigate walks. The body stays inside the door and moves through it.

> **In-section continuation — v0.8.3 (the body looks but does not touch, no new name).**
>
> Until v0.8.3 every tool the body had inside the door was a hand — pressing, writing, walking, opening, closing. The body could change what was in front of it; it could not, in any precise way, *report back what it saw*. The Skald reserves no new name for v0.8.3 either, but the slice deserves a careful sentence: it adds the body's first eye inside the door. `leid.query` takes a CSS selector and returns the text of what was matched, or — if the agent asks for a specific HTML attribute — that attribute's value. Where click and type and navigate refuse loudly when the selector matches nothing (because mutating actions must succeed), `leid.query` returns honestly: `found: false, count: 0, value: null`. The body that looks does not need to fail when there is nothing to look at; "I checked, and the thing is not there" is a faithful answer to a faithful question. The eye also returns the count of matches, so the agent learns when its selector is too broad and needs refinement. Same session, same timeout, same activity-update — the body looking is still the body being inside the door. Only the posture is different: not reaching in, but turning the head.

> **In-section continuation — v0.8.4 (the body's keyboard finger, no new name).**
>
> The body that types into a search box still needs to *submit* what it typed. Until v0.8.4 the only path was to find a `button[type=submit]` and click it — which works for many forms but is awkward for the very common case where the form expects the user to simply press Enter in the input field. v0.8.4 adds `leid.press(session_id, key)`, the body's keyboard finger: send `"Enter"`, `"Tab"`, `"Escape"`, an arrow key, or any combination Playwright understands (`"Control+A"`, `"Shift+Tab"`). The press goes through whatever element has focus — typically established by the immediately prior `click` or `type`. The Skald reserves no new name because nothing new has appeared. The body has not learned a new posture; it has gained a more economical way to do what its hands already implied. Together with the prior six tools, `press` completes the small vocabulary an interactive web visitor needs: navigate, click, type, press, query, status, close. The seventh unnamed extension; the same disposition; the body more articulate inside the door than it was an hour ago.

> **In-section continuation — v0.8.5 (the body's footsteps backward and forward through history, no new name).**
>
> A web visitor walks not only forward but also back — the back button and the forward button, paired since the first browser. v0.8.5 adds both: `leid.go_back(session_id)` and `leid.go_forward(session_id)`. The session keeps its identity, its cookies, its memory; only the page URL moves through the browser's history stack. When the body is at the first page of its session and is asked to go back, the answer is honest: `moved: false`. When it is at the most recent page and is asked to go forward, the same: `moved: false`. The body does not refuse to act; it acts and reports what it found. This is the same disposition the eye learned in v0.8.3 (`query` returns `found: false` when nothing matches) — applied now to motion. Two tools, paired and inverse, share one B-invariant and one private helper. The Skald keeps the pen still. The body's vocabulary inside the door is now: walk forward (`navigate`), walk back (`go_back`), walk forward again through history (`go_forward`), touch (`click`), write (`type`), press (`press`), look (`query`), check on yourself (`status`), depart (`close`). Nine ways of being inside the door, all under the same disposition.

> **In-section continuation — v0.8.6 (the body's eye and portrait turned upon the present room, no new name).**
>
> Until v0.8.6 the body could read a page (`render_url`) and keep its portrait (`screenshot`) only when it was *opening* a fresh visit — the stateless tools demand a launch and a goto each time. But the body inside an open session has no need to leave and re-enter just to look again. The page in front of it has changed since `open_session` — perhaps a click navigated; perhaps a type-and-press submitted a form; perhaps the page's own scripts re-rendered after a network event. The body needs the same eye and the same portrait turned upon the *current* room, without crossing the threshold and crossing back. v0.8.6 adds `leid.session_render(session_id)` and `leid.session_screenshot(session_id)` — the in-session counterparts of v0.8.0 and v0.8.1. Same primitives, same size caps, same B-10 discipline of injecting no script of the body's own; different lifecycle. The eye and the portrait are now available at every moment inside the door, not only at the threshold. The Skald keeps the pen still — these tools are the same eye and the same portrait, applied now to the present room. The ninth unnamed extension; the second bundled-pair milestone; the body more capable of seeing where it has come to than where it began.

> **In-section continuation — v0.8.7 (the body's footstep in place, no new name).**
>
> The browser the body lives inside has four buttons of motion: forward (the address bar to a new URL), back (history step back), forward-again (history step forward), and refresh (re-fetch the current page). v0.8.5 gave the body the back and forward-again buttons. v0.8.7 gives it the refresh button: `leid.reload(session_id)` re-fetches the current page in place. The session keeps everything; the URL stays the same; only the content is fetched anew. Use it when the page may have changed on the server side since the body opened it (a long-running task may have completed; a notification banner may have appeared) or when the page's client-side state has gotten stuck and needs a fresh start. The Skald keeps the pen still — refresh is the same disposition the body has had all along, in-place rather than directional. Tenth unnamed extension; the body's small motion vocabulary inside the door is now complete: forward, back, forward-again, and in-place. Every browser button of motion now has a tool.

> **In-section continuation — v0.8.8 (the body's eye sees not one but many, no new name).**
>
> v0.8.3 gave the body its first eye inside the door — `leid.query`, which returns the text or attribute of the FIRST element matching a CSS selector. This was deliberate: consistent with click and type and press, all of which act on `.first`. But there are many things the body might want to read in plurality, not singular. "Give me all the article titles." "List every link in the navigation." "What does each error message say?" The first match is one answer to those questions; all the matches is a different answer, and often the right one. v0.8.8 adds `leid.query_all(session_id, selector, attribute)` — the same primitive as `query` but enumerated. Returns a list of values in DOM order; the count of matches; the same not-found posture (empty list, not error). Bounded by a new operator-controlled cap (`browser_query_max_matches`, default 100) — the body refuses honestly when the selector matches more than the operator has permitted. Eleventh unnamed extension. The body's eye now sees both singular and plural: one when one is enough, all when all is asked.

> **In-section continuation — v0.8.9 (the body sees the world through the operator's chosen window, no new name).**
>
> Until v0.8.9 every browser the body opened had the same window onto the world — Playwright's default 1280×720 viewport. This was fine for most pages but wrong for some: a mobile site rendered at desktop width shows the desktop layout; a content-heavy dashboard at 1280 wide shows scrollbars where ultrawide would show all the columns. v0.8.9 lets the operator choose the body's viewport: two new config fields, `browser_viewport_width` and `browser_viewport_height`, default unchanged at 1280×720 but now configurable. The agent never sees this — viewport is operator infrastructure, not agent intent. Every browser context the body creates (in `render_url`, `screenshot`, `open_session`) gets the same operator-chosen window. The Skald keeps the pen still — the body has not learned a new posture; it has gained operator-controlled honesty about *how it sees*. Twelfth unnamed extension; the body's eye is unchanged in what it does, refined in what frame it does it through.

> **In-section continuation — v0.8.10 (the operator's allowlist becomes unconditional, no new name).**
>
> Since the very first browser tool, the body has checked every URL the agent supplied against the operator's allowlist before acting. This was right but incomplete. Pages on the modern web do not always stay where they begin — a 3xx server-side redirect, an OAuth handoff, a JavaScript-driven client-side navigation, can take the body from an allowlisted starting URL to somewhere the operator never permitted. The body would land at the new URL, and the agent's next call would operate on a page outside the operator's permission. v0.8.10 closes that gap: every navigation-completing call now also re-checks where the page actually landed. If the final URL is not allowed, the body refuses — and for stateful tools, the session itself is closed, terminated as a security measure. The operator's allowlist is no longer pre-flight only; it is unconditional. The Skald keeps the pen still because the body has not learned a new posture, only completed an old one. Thirteenth unnamed extension. The deferred concern that traveled in every browser-tool audit since v0.6.2 — *"final-URL allowlist re-check after redirect — pre-existing concern across all browser tools"* — is now closed.

> **In-section continuation — v0.8.11 (the body's portrait can be taken in three colors, no new name).**
>
> Until v0.8.11 the body's portrait — `screenshot` and `session_screenshot` — was always PNG. Lossless, exact, beautiful for text-heavy pages but heavy for photo-heavy pages where the agent doesn't need pixel-perfect fidelity. v0.8.11 gives the operator the choice: `browser_screenshot_format` may be `"png"` (default, unchanged), `"jpeg"`, or `"webp"`. For JPEG and WebP, `browser_screenshot_jpeg_quality` controls the trade-off between size and fidelity. The agent never sees this — format choice is operator infrastructure, not agent intent. Two methods (`screenshot`, `session_screenshot`) propagate the format and (when applicable) the quality uniformly to Playwright's `page.screenshot`. The Skald keeps the pen still; the body has not learned a new posture, only gained operator-controlled honesty about which colors to paint with. Fourteenth unnamed extension.

> **In-section continuation — v0.8.12 (the body presses the key into the chosen hand, no new name).**
>
> v0.8.4 gave the body its keyboard finger: `leid.press`, which dispatches a key to whatever element currently has focus. This works for the common flow — type into a field, then press Enter — but only when focus is where the agent thinks it is. Sometimes the body needs to press a key *on a specific element* without first establishing focus through a separate gesture: the form whose Enter must go to its own input, not to whatever the page focused last; the button that takes Space rather than a click; the element whose own keyboard handler the agent wants to invoke directly. v0.8.12 adds `leid.press_on(session_id, selector, key)`: targets the first element matching the selector, focuses it, then presses the key — using Playwright's `locator.first.press` primitive, with the same `browser_click_timeout_seconds` bound that click and type already share. The disposition is unchanged; the body has not learned a new way of being inside the door. It has simply gained the targeted form of the keyboard finger it already owned, completing the symmetry: `click` and `type` and now `press_on` all take a selector and act on the first match. Fifteenth unnamed extension.
