...
| `skilningr::minni::tool_call(state, tool_name, call_id)` — Minni filesystem sense milestones | `sense.tool_call` (sense="minni") | S->C | §3.9 |
| `skilningr::skepja::tool_call(state, tool_name, call_id)` — Skepja terminal sense milestones | `sense.tool_call` (sense="skepja") | S->C | §3.9 |
| `skilningr::leid::tool_call(state, tool_name, call_id)` — Leið HTTP fetch sense milestones | `sense.tool_call` (sense="leid") | S->C | §3.9 |
| `skilningr::library::tool_call(state, tool_name, call_id)` — Library (Mímisbrunnr) sense milestones | `sense.tool_call` (sense="library") | S->C | §3.9 |
...
### 8.4 Sense naming bridge — sense_id to True Name to tool prefix (v0.7)

All senses follow the same wire pattern: the `sense` field in `sense.tool_call`
events (§3.9) and the `sense_id` field in `toggle_sense` commands (§4.5) use the
code-facing identifier (transliterated True Name, no diacritics, lowercase).

| sense_id (wire) | True Name | Layer | Tool prefix | Config key | Status |
|---|---|---|---|---|---|
| `smidja` | Smiðja ("smithy") | L5.5 | `smidja.*` | `skilningr.smidja.*` | Live (v0.6+) |
| `minni` | Minni ("memory") | L5.1 | `minni.*` | `skilningr.minni.*` | Scaffolded (v0.6.2) |
| `skepja` | Skepja ("shaping") | L5.2 | `skepja.*` | `skilningr.skepja.*` | Scaffolded (v0.6.2) |
| `leid` | Leið ("path/way") | L5.3 | `leid.*` | `skilningr.leid.*` | Scaffolded (v0.6.2) |
| `library` | Mímisbrunnr ("Mímir's well") | L5.9 | `library.*` | `skilningr.library.*` | Scaffolded (v0.7) |
| `hlust` | Hlust ("hearing") | L2 | — | `hlust.*` | Live (v0.4+) |
| `tunga` | Tunga ("tongue") | L3 | — | `tunga.*` | Live (v0.4+) |
| `sjon` | Sjón ("vision") | L3 | — | `sjon.*` | Live (v0.5+) |
| `filesystem` | Minni (deprecated alias) | L5.1 | — | — | Legacy toggle_sense alias only |
| `terminal` | Skepja (deprecated alias) | L5.2 | — | — | Legacy toggle_sense alias only |
| `browser` | Leið (deprecated alias) | L5.3 | — | — | Legacy toggle_sense alias only |

Note on deprecated aliases: the `toggle_sense` §4.5 command previously accepted
`"filesystem"`, `"terminal"`, and `"browser"` as `sense_id` values (per the original
stub list in SkilningrConfig). As of v0.6.2, the canonical identifiers are `"minni"`,
`"skepja"`, and `"leid"`. The old names are kept in the §4.5 valid-values list for
backwards compatibility with any frontend code that referenced them, but new code
must use the canonical sense_id values. Forge Wave 2 will add routing for both.
