...
```
  ...
  [heretic.yaml]  [L0 Grunnr]
       ...
       |    rodd.stt_backend   --> L2 Rödd (Hlust)
       |    rodd.tts_endpoint  --> L2 Rödd (Tunga)
  ...
  [Holdvörðr initializes layers in sequence]
       ...
       |       sense.auga         (screen + vision)
       |       sense.hlust        (STT — wraps Whisper.cpp)
       |       sense.tunga        (TTS — proxies to ChatterBox)
       |       sense.hond         (Photopea bridge, if enabled)
       |       sense.smidja       (Blender/Seidr-Smidja, if enabled)
       |       sense.leid         (Browser MCP, if enabled)
       |       sense.minni        (FileSystem MCP)
       |       sense.skepja       (Terminal MCP)
       |       sense.bod          (AgentMail MCP, if enabled)
       |       sense.likami       (VRChat MCP, if enabled)
       |       sense.mimisbrunnr  (Library MCP, if enabled and downloaded)
       |       sense.nyr_limr     (custom plugin MCPs, if configured)
       |
       v
...
**Open question:** Does Whisper.cpp model loading happen at Kynding or is it lazy-loaded at
first audio? If lazy-loaded, Kynding is faster but the first voice turn has extra latency.
Recommend: lazy-load with pre-warm option in config.
...
**Open question:** Should the system prompt carry a manifest of available senses (which MCPs are
enabled) so the agent knows from first contact what tools it can call? Recommendation: yes —
Holdvörðr should generate a `senses_manifest` object and inject it into the system prompt at
Tengsl time. This is a design decision for the Architect to formalize in LAYER_INTERFACES.md.
...
```
  [Agent returns tool_call in response]
       ...
       |    "tool_calls": [{
       |      "function": {
       |        "name": "sense.smidja.execute_blender_script",
       |        "arguments": "{\"script\": \"...python...\", \"wait\": true}"
       |      }
       |    }]
       |  }
       v
  [L1 Bifröst client: tool call dispatcher]
       |
       |  parses tool name: "sense.smidja.*" --> route to Skilningr/Smiðja
       |
       v
  [L5 Skilningr: MCP sense router]
       |
       |  looks up registered sense by prefix
       |  forwards call to appropriate MCP server process
       v
  [sense.smidja — Smiðja MCP server]  (local process, laptop)              (wrm/cld)
       |
...
- `sense.minni` (FileSystem): hot — local disk, <60ms for most operations
- `sense.skepja` (Terminal): warm — depends on command, typically <1200ms for simple ops
- `sense.auga` (screenshot): hot for capture, warm for analysis if sent to agent for vision
- `sense.smidja` (Blender via Seidr-Smidja): warm to cold — depends on operation complexity
- `sense.leid` (Browser): warm — network-dependent
- `sense.mimisbrunnr` (Library search): warm for keyword, cold for vector search
...
```
  [Agent tool_call: sense.auga.screenshot]
  OR
...
### 4.4 Library Search Flow — Mímisbrunnr / sense.mimisbrunnr

When the agent needs to search the offline well of wisdom.

```
  [Agent tool_call: sense.mimisbrunnr.search]
  |  arguments: {"query": "What does Völuspá say about Ragnarök?", "source": "norse_sagas"}
  v
  [L5 Skilningr: sense.mimisbrunnr MCP server]                              (wrm/cld)
       |
       ...
       |
       v
  [sense.mimisbrunnr returns MCP tool result]
       |
       |  format: {
       |    "results": [
       |      {
...
```
  ...
  T+900ms     Agent returns:
              {tool_calls: [{function: {name: "sense.smidja.execute_blender_script",
                             arguments: "{\"script\": \"create_shield_mesh.py\"}"}}]}

  T+900ms     L1 Bifröst dispatches to L5 Skilningr → sense.smidja
  ...
  T+5500ms    Session log event written:
              {type:"voice_turn_with_tool", transcript:"...", tool:"sense.smidja",
...
```
  ...
  Skilningr → sense.smidja outward Blender tool call               MCP call (JSON)
  sense.smidja → Brúarhönd outward remote desktop dispatch         HTTP POST JSON
  Brúarhönd → sense.smidja inward  operation result                HTTP response JSON
  Skilningr → sense.mimisbrunnr outward library query              {query, source_filter}
  libzim/MindSpark → sense.mimisbrunnr inward search results       list of {title, excerpt, attribution}
```
...
```
  ...
  [Skilningr: MCP dispatcher]
       |
       |  parses tool name: "sense.<server>.<method>"
       |  looks up registered MCP server for <server>
       |  forwards: method + arguments JSON
       |
       |-- sense.auga       --> Sjón screen capture / browser control
       |-- sense.hlust      --> Whisper.cpp (for on-demand transcription, beyond VAD pipeline)
       |-- sense.tunga      --> ChatterBox TTS proxy
       |-- sense.hond       --> Photopea bridge (HTTP or WebSocket to Photopea)
       |-- sense.smidja     --> Seidr-Smidja Brúarhönd client
       |-- sense.leid       --> Browser automation (Playwright or equivalent)
       |-- sense.minni      --> FileSystem operations (read/write/list)
       |-- sense.skepja     --> Terminal subprocess runner
       |-- sense.bod        --> AgentMail SMTP/IMAP client
       |-- sense.likami     --> VRChat OSC/API bridge
       |-- sense.mimisbrunnr --> Library search backend
       |-- sense.nyr_limr/* --> Custom plugin MCP server(s)
       |
...
```
  COLD PATH — Library indexing (sense.mimisbrunnr)                           (cld)
  ----------------------------------------------------------------
