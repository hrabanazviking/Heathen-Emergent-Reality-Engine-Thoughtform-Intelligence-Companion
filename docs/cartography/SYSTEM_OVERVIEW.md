...
```
  ...
  v0.7  First Drink at the Well   Mímisbrunnr    L5.9 light tier (IN PROGRESS)
        |  Codename: "First Drink at the Well" — the spirit drinks of the Norse corpus.
        |  v0.7 scope: LIGHT TIER ONLY
        |    NEW: src/heretic/skilningr/mimisbrunnr/ subsystem
        |         manifest.py — NorseStarterPackManifest (5 Norse sources, public domain)
        |         downloader.py — async httpx + SHA-256 streaming verify + atomic write
        |         store.py — cross-platform storage layout (dirs library; ~/.local/share/heretic/library/)
        |         index.py — stdlib re-based line-by-line keyword search; line-offset index
        |         consent.py — per-source confirmation prompt (or --yes flag)
        |         errors.py — LibraryError hierarchy (7 failure modes; see DATA_FLOW.md §4.14)
        |    NEW: src/heretic/skilningr/senses/library/ — LibrarySense (L5.9)
        |         3 tools: library.search, library.get_text, library.list_sources
        |         LibraryClient delegates to mimisbrunnr/ backend
        |         LibraryConfig: enabled: false default
        |    NEW: heretic library CLI subcommand
        |         heretic library list | download <source_id> | remove <source_id> | rebuild-index
        |    PRIVACY INVARIANTS:
        |         disabled by default; per-source consent; LOCAL search only; offline during queries
        |    See DATA_FLOW.md §4.14 (library flow — three agent tool paths)
        |        DATA_FLOW.md §4.14.1 (operator download flow — consent + SHA-256 + atomic)
        |        DATA_FLOW.md §4.14.2 (storage layout — cross-platform user-data dirs)
        |        DATA_FLOW.md §4.14.3 (privacy invariants — 7 sealed invariants)
        |        DATA_FLOW.md §16 v0.7 (five-sense diagram + mimisbrunnr/ module map)
        |  v0.7 does NOT add:
        |    vector search / embeddings (v0.9 — [library-vector] extra)
        |    ZIM ingest / Wikipedia (v0.8)
        |    MindSpark backend (v0.10)
        |    auto-download on first run (operator must confirm each source)
  v0.7.5 (MERGED INTO v0.7 — Mímisbrunnr was the v0.7.5 spec; promoted to v0.7)
